            ...
            for (til::CoordType j = 0; j < size.X; ++j, ++outIter)
            {
                ...
                if (WI_IsFlagSet(in1.Attributes, COMMON_LVB_LEADING_BYTE))
                {
                    ...
                    if (j < size.X - 1)
                    {
                        ...
                        in1.Char.AsciiChar = AsciiDbcs[0];
                        in2.Char.AsciiChar = AsciiDbcs[1];
                    }
                    else
                    {
                        ...
                        in1.Char.AsciiChar = UNICODE_SPACE;
                ...
                else if (WI_AreAllFlagsClear(in1.Attributes, COMMON_LVB_SBCSDBCS))
                {
                    ...
                    const auto wch = in1.Char.UnicodeChar;
                    ConvertToOem(codepage, &wch, 1, &in1.Char.AsciiChar, 1);
                }
            }
        }
            ...
            for (til::CoordType j = 0; j < size.X; ++j, ++outIter)
            {
                ...
                if (IsDBCSLeadByteConsole(in1.Char.AsciiChar, &gci.OutputCPInfo))
                {
                    ...
                    if (j < size.X - 1)
                    {
                        ...
                        in2.Char.UnicodeChar = UNICODE_DBCS_PADDING;
                    }
                    else
                    {
